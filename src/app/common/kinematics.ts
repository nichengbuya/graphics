...
class Kinematics {
  ...
  createTransformMatrix(theta: number, d: number, a: number, alpha: number) {
  ...
  ur_inverse(cart_pos: any[]) {
    const nx = cart_pos[0]; const ox = cart_pos[1]; const ax = cart_pos[2]; const px = cart_pos[3];
    const ny = cart_pos[4]; const oy = cart_pos[5]; const ay = cart_pos[6]; const py = cart_pos[7];
    const nz = cart_pos[8]; const oz = cart_pos[9]; const az = cart_pos[10]; const pz = cart_pos[11];
    const a = [0, -0.425, -0.39225, 0, 0, 0];
    const d = [0.089159, 0, 0, 0.10915, 0.09465, 0.0823];
    const temp1 = d[5] * ay - py;
    const temp2 = d[5] * ax - px;

    const jnt1_pos0 = Math.atan2(temp1, temp2) - Math.atan2(d[3], Math.sqrt(temp1 * temp1 + temp2 * temp2 - d[3] * d[3]));
    const jnt1_pos1 = Math.atan2(temp1, temp2) - Math.atan2(d[3], -Math.sqrt(temp1 * temp1 + temp2 * temp2 - d[3] * d[3]));

    const jnt5_pos0 = this.SolveTheta5(jnt1_pos0, ax, ay);
    const jnt5_pos1 = -jnt5_pos0;
    const jnt5_pos2 = this.SolveTheta5(jnt1_pos1, ax, ay);
    const jnt5_pos3 = -jnt5_pos2;

    const jnt6_pos0 = this.SolveTheta6(jnt1_pos0, jnt5_pos0, ox, oy, nx, ny);
    const jnt6_pos1 = this.SolveTheta6(jnt1_pos0, jnt5_pos1, ox, oy, nx, ny);
    const jnt6_pos2 = this.SolveTheta6(jnt1_pos1, jnt5_pos2, ox, oy, nx, ny);
    const jnt6_pos3 = this.SolveTheta6(jnt1_pos1, jnt5_pos3, ox, oy, nx, ny);


    const jnt3_jnt2_pos01 = this.SolveTheta3AndTheta2(jnt1_pos0, jnt5_pos0, jnt6_pos0, a[1], a[2], d[0], d[4], d[5], nx, ny, nz, ox, oy, oz, ax, ay, az, px, py, pz);
    const jnt3_jnt2_pos23 = this.SolveTheta3AndTheta2(jnt1_pos0, jnt5_pos1, jnt6_pos1, a[1], a[2], d[0], d[4], d[5], nx, ny, nz, ox, oy, oz, ax, ay, az, px, py, pz);
    const jnt3_jnt2_pos45 = this.SolveTheta3AndTheta2(jnt1_pos1, jnt5_pos2, jnt6_pos2, a[1], a[2], d[0], d[4], d[5], nx, ny, nz, ox, oy, oz, ax, ay, az, px, py, pz);
    const jnt3_jnt2_pos67 = this.SolveTheta3AndTheta2(jnt1_pos1, jnt5_pos3, jnt6_pos3, a[1], a[2], d[0], d[4], d[5], nx, ny, nz, ox, oy, oz, ax, ay, az, px, py, pz);
    const jnt3_pos0 = jnt3_jnt2_pos01[0];
    const jnt3_pos1 = jnt3_jnt2_pos01[2];
    const jnt3_pos2 = jnt3_jnt2_pos23[0];
    const jnt3_pos3 = jnt3_jnt2_pos23[2];
    const jnt3_pos4 = jnt3_jnt2_pos45[0];
    const jnt3_pos5 = jnt3_jnt2_pos45[2];
    const jnt3_pos6 = jnt3_jnt2_pos67[0];
    const jnt3_pos7 = jnt3_jnt2_pos67[2];

    const jnt2_pos0 = jnt3_jnt2_pos01[1];
    const jnt2_pos1 = jnt3_jnt2_pos01[3];
    const jnt2_pos2 = jnt3_jnt2_pos23[1];
    const jnt2_pos3 = jnt3_jnt2_pos23[3];
    const jnt2_pos4 = jnt3_jnt2_pos45[1];
    const jnt2_pos5 = jnt3_jnt2_pos45[3];
    const jnt2_pos6 = jnt3_jnt2_pos67[1];
    const jnt2_pos7 = jnt3_jnt2_pos67[3];

    const jnt4_pos0 = this.SolveTheta4(jnt1_pos0, jnt2_pos0, jnt3_pos0, jnt5_pos0, jnt6_pos0, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos1 = this.SolveTheta4(jnt1_pos0, jnt2_pos1, jnt3_pos1, jnt5_pos0, jnt6_pos0, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos2 = this.SolveTheta4(jnt1_pos0, jnt2_pos2, jnt3_pos2, jnt5_pos1, jnt6_pos1, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos3 = this.SolveTheta4(jnt1_pos0, jnt2_pos3, jnt3_pos3, jnt5_pos1, jnt6_pos1, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos4 = this.SolveTheta4(jnt1_pos1, jnt2_pos4, jnt3_pos4, jnt5_pos2, jnt6_pos2, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos5 = this.SolveTheta4(jnt1_pos1, jnt2_pos5, jnt3_pos5, jnt5_pos2, jnt6_pos2, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos6 = this.SolveTheta4(jnt1_pos1, jnt2_pos6, jnt3_pos6, jnt5_pos3, jnt6_pos3, nx, ny, nz, ox, oy, oz, ax, ay, az);
    const jnt4_pos7 = this.SolveTheta4(jnt1_pos1, jnt2_pos7, jnt3_pos7, jnt5_pos3, jnt6_pos3, nx, ny, nz, ox, oy, oz, ax, ay, az);
  ...
  SolveTheta2(theta1: number, theta3: number, theta5: number, theta6: number, n: number, m: number, a1: number, a2: number, nz: any) {
    ...
    const A = (n * (a1 + a2 * c3) - m * a2 * s3) / (a1 * a1 + a2 * a2 + 2 * a1 * a2 * c3);
    const B = (m + a2 * s3 * A) / (a2 * c3 + a1);
    const theta2 = Math.atan2(A, B);
    return theta2;
  }
  SolveTheta3AndTheta2(theta1: number, theta5: number, theta6: number, a1: number, a2: number, d0: number, d4: number, d5: number, nx: number, ny: number, nz: number, ox: number, oy: number, oz: number, ax: number, ay: number, az: number, px: number, py: number, pz: number) {
    const s1 = Math.sin(theta1);
    const c1 = Math.cos(theta1);
    const s6 = Math.sin(theta6);
    const c6 = Math.cos(theta6);
    const m = d4 * ((c1 * nx + s1 * ny) * s6 + (c1 * ox + s1 * oy) * c6) - d5 * (c1 * ax + s1 * ay) + c1 * px + s1 * py;
    const n = d4 * (s6 * nz + c6 * oz) - d5 * az + pz - d0;
    // let theta3 = new Array(2);
    // theta3 condition:  m^2+n^2<=(a(2)+a(3))^2

    const theta3_0 = Math.acos((m * m + n * n - a1 * a1 - a2 * a2) / (2 * a1 * a2));
    const theta3_1 = -theta3_0;
    const theta2_0 = this.SolveTheta2(theta1, theta3_0, theta5, theta6, n, m, a1, a2, nz);
    const theta2_1 = this.SolveTheta2(theta1, theta3_1, theta5, theta6, n, m, a1, a2, nz);
    const theta3_theta2 = new Array(4);
  ...
  SolveTheta5(theta1: number, ax: number, ay: number) {
    const c1 = Math.cos(theta1);
    const s1 = Math.sin(theta1);
    const theta5_0 = Math.acos(s1 * ax - c1 * ay);
    const thate5_1 = -theta5_0;
    // let theta5 = [theta5_0, theta5_1];
    return theta5_0;
  }

  SolveTheta6(theta1: number, theta5: number, ox: number, oy: number, nx: number, ny: number) {
    const s1 = Math.sin(theta1);
    const c1 = Math.cos(theta1);
    const s5 = Math.sin(theta5);
    // let c5 = Math.cos(theta5);
    const theta6 = Math.atan2((-s1 * ox + c1 * oy) / s5, (-c1 * ny + s1 * nx) / s5);
    return theta6;
  }



  SolveTheta4(theta1: number, theta2: number, theta3: number, theta5: number, theta6: number, nx: number, ny: number, nz: number, ox: number, oy: number, oz: number, ax: number, ay: number, az: number) {
    const s1 = Math.sin(theta1);
    const c1 = Math.cos(theta1);
    const s5 = Math.sin(theta5);
    const c5 = Math.cos(theta5);
    const s6 = Math.sin(theta6);
    const c6 = Math.cos(theta6);
    const C = (nz * c6 - oz * s6) * c5 - az * s5;
    const D = ((c1 * nx + s1 * ny) * c6 - (c1 * ox + s1 * oy) * s6) * c5 - (c1 * ax + s1 * ay) * s5;
    const theta4 = Math.atan2(C, D) - theta2 - theta3;
    return theta4;
  }
  ...
  aubo_inverse(cart_pos: any[]) {
    ...
    const q_sols = Array.from({ length: 6 }, x => Array.from({ length: 8 }, y => 0));
    let num_sols = 0;
    ...
    ////////////////////////////// wrist 2 jolet (q5) //////////////////////////////
    // MatrixXd q5(2, 2);
    const q5 = Array.from({ length: 2 }, x => Array.from({ length: 2 }, y => 0));
    ...
    const q3 = new Array(2);
    const q2 = new Array(2);
    const q4 = new Array(2);
  ...
  antiSinCos(sA: number, cA: number) {
    const eps = 1e-8;
    let angle = 0;
  ...
  reverseMatrix(sourceArr: string | any[]) {
  ...
  CrossProduct(v1: THREE.Vector3, v2: THREE.Vector3) {
  ...
  RotationMatrix(angle: number, u: { x: number; y: number; z: number; }) {
    // double norm = Normalize(u);
    const rotatinMatrix = new THREE.Matrix4();
