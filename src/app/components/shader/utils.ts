...
uniform vec2 iMouse;
uniform vec2 iResolution;
uniform float iVelocityBox;
uniform float iProgress;
uniform float iAngle;
uniform float iDistance;
uniform float iVelocitySphere;
...
float movingSphere(vec3 p,float shape){
    float rad=iAngle*PI;
    vec3 pos=vec3(cos(rad),sin(rad),0.)*iDistance;
    vec3 displacement=pos*fract(iTime*iVelocitySphere);
    float gotoCenter=sdSphere(p-displacement,.1);
    return smin(shape,gotoCenter,.3);
}

float sdf(vec3 p){
    vec3 p1=rotate(p,vec3(1.),iTime*iVelocityBox);
    float box=sdBox(p1,vec3(.3));
    float sphere=sdSphere(p,.3);
    float sBox=smin(box,sphere,.3);
    float mixedBox=mix(sBox,box,iProgress);
...
void main(){
    ...
    if(depth<end){
        ...
        color=texture2D(iTexture,matcapUv).rgb;
